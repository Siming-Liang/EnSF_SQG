...
hcovlocal_scale = 2800*1000 #float(sys.argv[1])

covinflate1 = 0.8
covinflate2 = -1
exptname = os.getenv('exptname','EnSF_arctan')
...
for ntime in range(nassim): #nassim
    ...
    for k in range(2):
        # surface temp obs
        pvob[k] = np.arctan(scalefact*pv_truth[ntime+ntstart,k,:,:].ravel()[indxob]) + 1 * rsobs.normal(scale=oberrstdev,size=nobs) # add ob errors
    xob = x.ravel()[indxob]
    yob = y.ravel()[indxob]
    ...
    if direct_insertion and nobs == nx*ny:
        ...
        if global_enkf and not use_letkf:
            ...
            xens =\
            EnSF_Update.state_update_arctan(x_input = xens.reshape(nanals,2*nx*ny),state_target_input = pv_truth[ntime+ntstart].reshape(2*nx*ny), obs_input = pvob.reshape(2*nobs))
            xens = xens.cpu().numpy()
