...
hcovlocal_scale = 2200*1000 #float(sys.argv[1])

covinflate1 = 0.3
covinflate2 = -1
exptname = os.getenv('exptname','Jump_EnSF_64_random')
...
rsjump = np.random.RandomState(24) # fixed seed for observations
...
for ntime in range(nassim): #nassim
    ...
    EnSF_Update = EnSF(n_dim = nx*ny*2, ensemble_size = nanals ,eps_alpha=0.05, device= 'cuda' ,\
                   obs_sigma = oberrstdev, euler_steps = 1000, scalefact = nc_climo.f*nc_climo.theta0/nc_climo.g, init_std_x_state = init_std_x_state,  ISarctan=False)
    ...
    if direct_insertion and nobs == nx*ny:
        ...
        if global_enkf and not use_letkf:
            ...
            xens =\
            EnSF_Update.state_update_normalized(x_input = xens.reshape(nanals,2*nx*ny),state_target_input = pv_truth[ntime+ntstart].reshape(2*nx*ny), obs_input = pvob.reshape(2*nx*ny))
            xens = xens.cpu().numpy()
