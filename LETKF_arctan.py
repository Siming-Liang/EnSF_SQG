...
hcovlocal_scale = 2800* 1000
covinflate1 = 0.8
covinflate2 = -1

exptname = os.getenv('exptname','LETKF_arctan_hloc{}km_inf{}.nc'.format(hcovlocal_scale/1000 ,covinflate1))
threads = int(os.getenv('OMP_NUM_THREADS','1'))

runing_rmse = 'final_arc_rmse_{}_{}'.format(hcovlocal_scale/1000 ,covinflate1)
...
if savedata is not None:
   ...
   err_b_modSpace_nc = nc.createVariable('err_b_modSpace',np.float32,('t',))
   spread_b_modSpace_nc = nc.createVariable('spread_b_modSpace',np.float32,('t',))
   err_a_modSpace_nc = nc.createVariable('err_a_modSpace',np.float32,('t',))
   spread_a_modSpace_nc = nc.createVariable('spread_a_modSpace',np.float32,('t',))
   ...
   err_b_modSpace_nc.units = 'K'
   spread_b_modSpace_nc.units = 'K'
   err_a_modSpace_nc.units = 'K'
   spread_a_modSpace_nc.units = 'K'
...
for ntime in range(nassim): #nassim
    ...
    for k in range(2):
        # surface temp obs
        pvob[k] = np.arctan(scalefact*pv_truth[ntime+ntstart,k,:,:].ravel()[indxob]) + rsobs.normal(scale=oberrstdev,size=nobs) # add ob errors
    xob = x.ravel()[indxob]
    yob = y.ravel()[indxob]
    ...
    pverr_b_modSpace = (scalefact*(pvensmean_b-pv_truth[ntime]))**2
    pverr_b_modSpace = np.sqrt(pverr_b_modSpace.mean())
    pvsprd_b_modSpace = ((scalefact*(pvensmean_b-pvens))**2).sum(axis=0)/(nanals-1)
    pvsprd_b_modSpace = np.sqrt(pvsprd_b_modSpace.mean())
    ...
    if savedata is not None:
        if savedata == 'restart' and ntime != nassim-1:
            ...
            #x_obs[ntime] = xob
            #y_obs[ntime] = yob
            err_b_modSpace_nc[ntime-nassim_write] = pverr_b_modSpace
            spread_b_modSpace_nc[ntime-nassim_write] = pvsprd_b_modSpace 
    ...
    pverr_a_modSpace = (scalefact*(pvensmean_a-pv_truth[ntime]))**2
    pverr_a_modSpace = np.sqrt(pverr_a_modSpace.mean())
    pvsprd_a_modSpace = ((scalefact*(pvensmean_a-pvens))**2).sum(axis=0)/(nanals-1)
    pvsprd_a_modSpace = np.sqrt(pvsprd_a_modSpace.mean())
    ...
    if savedata is not None:
        if savedata == 'restart' and ntime != nassim-1:
            ...
            err_a_modSpace_nc[ntime-nassim_write] = pverr_a_modSpace
            spread_a_modSpace_nc[ntime-nassim_write] = pvsprd_a_modSpace
            nc.sync()
