...
def enkf_update(
    ...
    if obcovlocal is not None:  # serial EnSRF update
        ...
        hx = np.empty((nanals, 2 * nobs), float)
        omf = np.empty(2 * nobs, float)
        oberrvar = np.empty(2 * nobs, float)
        covlocal_tmp = np.empty((2 * nobs, 2, ndim1), float)
        for kob in range(2):
            fact[:] = 1.0
            fact[1 - kob] = vcovlocal_fact
